...
class AdminController extends Controller {
    ...
    public function displayAdminPage(Request $request) {
        if (!$this->isLoggedIn()) {
            return redirect(route('login'))->with('error', 'Inicia sesión para acceder ao panel.');
        }
        ...
        if (!$user) {
            return redirect(route('index'))->with('error', 'Conta non válida ou desactivada.');
        }
    ...
    public function changePassword(Request $request) {
        ...
        if (UserHelper::checkCredentials($username, $old_password) == false) {
            // Invalid credentials
            return redirect('admin')->with('error', 'Contrasinal actual incorrecta. Inténtao de novo.');
        }
        else {
            ...
            $request->session()->flash('success', "Contrasinal cambiada con éxito.");
